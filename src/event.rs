    ...
    I: Iterator<Item = Result<u8, Error>>,
{
    ...
    let err_would_block = Error::new(ErrorKind::WouldBlock, "Not enough input");
    match item {
        b'\x1B' => {
            ...
            Ok(match iter.next() {
                Some(Ok(b'O')) => {
                    match iter.next() {
                        ...
                        None => return Err(err_would_block),
                        _ => return Err(error),
                    }
                }
                Some(Ok(b'[')) => {
...
#[cfg(test)]
#[test]
fn test_parse_event_would_block() {
    let result: Result<_, Error> = parse_event(b'\x1B', &mut std::iter::empty());
